...
class SPORTidentEmulator(Thread):
    KNOWN_MSGS = {
        ...
        b"\x02\x83\x02\x74\x01\x04\x14\x03": [
            b"\x02\x83\x04\x00\x09\x74\x05\x31\xc7\x03"
        ],
        ...
        while not self.stop_event.is_set():
            ...
            if ser.in_waiting:
                ...
                if data[:3] == b"\xaa\xbb\xff":
                    if data[3] == 0x28:
                        print(
                            "Switching Huichang master station to Online/Offline mode"
                        )
                        connected = True
                    # Send Ok
...
def start_socat():
    ...
    process = subprocess.Popen(
        socat_command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, shell=True
    )
...
@pytest.mark.skipif(
    platform.system() != "Linux", reason="This test only works on Linux"
)
