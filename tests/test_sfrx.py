...
def _test_person_data(test_case):
    ...
    for day in range(2):
        ...
        assert person.organization.name == expected['org_name'], "Organization name error"
        ...
        assert person.group.course.name == day_expected['course_name'], "Course name error"
        assert result.start_time == hhmmss_to_time(day_expected['start']), "Start error"
        assert result.finish_time == hhmmss_to_time(day_expected['finish']), "Finish error"
