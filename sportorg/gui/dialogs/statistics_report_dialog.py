...
from sportorg.models.start.start_calculation import get_entries_statistics_data
...
class StatisticsReportDialog(QDialog):
    ...
    def init_ui(self):
        self.setWindowTitle(_('Entries statistics'))
        ...
        self.item_template.addItems(get_templates(config.template_dir('entries_statistics')))
    ...
    def apply_changes_impl(self):
        ...
        template = get_text_from_file(template_path, **get_entries_statistics_data())
        ...
        if _settings['save_to_last_file']:
            ...
            file_name = get_save_file_name(_('Save As HTML file'), _("HTML file (*.html)"),
                                           '{}_entries_statistics'.format(time.strftime("%Y%m%d")))
        if file_name:
