...
class MaxDialog(QDialog):
    ...
    def init_ui(self):
        self.setWindowTitle(translate("MAX"))
    ...
    def set_values(self):
        obj = race()
        token = settings.SETTINGS.max_token
        chat_id = obj.get_setting("max_chat_id", "")
        parse_mode = obj.get_setting("max_parse_mode", "")
        template = obj.get_setting("max_template", "{group} {name} {bib} {result} {place}")
        max_enabled = obj.get_setting("max_enabled", False)

        self.item_chat_id.setText(chat_id)
        self.item_token.setText(token)
        self.item_template.setText(template)
        self.item_enabled.setChecked(max_enabled)
    ...
    def apply_changes_impl(self):
        ...
        obj = race()
        settings.SETTINGS.max_token = self.item_token.text()
        obj.set_setting("max_chat_id", self.item_chat_id.text())
        obj.set_setting("max_enabled", self.item_enabled.isChecked())
        obj.set_setting("max_parse_mode", parse_mode)
        obj.set_setting("max_template", self.item_template.toPlainText())
