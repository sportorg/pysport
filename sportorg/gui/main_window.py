...
class MainWindow(QMainWindow):
    def __init__(self, argv=None):
        ...
        self.file_locker = pylocker.FACTORY(key='sportorg_locker_key', password='str(uuid.uuid1())', autoconnect=False)
                ...
                if is_new and os.path.exists(file_name):
                    if os.path.getsize(file_name) > 1000:
                        QMessageBox.warning(
                            self,
                            translate('Error'),
                            translate('Cannot overwrite existing file with new') + ': ' + file_name,
                        )
                        return
