...
versoin: 4 Bytes (Unsigned Long)
size: 8 Bytes (Insigned Long long)
...
class Header:
    ...
    def unpack_header(self, header):
        self.packTag, self.opType, self.objType, self.uuid, self.version, self.size = struct.unpack(Header.header_struck, header)
    ...
    def pack_header(self, psize):
        self.size = psize
        return struct.pack(Header.header_struck, self.packTag, self.opType, self.objType, bytes(self.uuid, 'ascii'), self.version, self.size)
