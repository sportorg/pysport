...
class SRPid(object):
    ...
    def _send_command(self, code, wait_response=True, timeout=None):
        ...
        cs = self._CRCsum(code + b'\0' + b'\0' + b'\0' + b'\0' + b'\0' + b'\0' + b'\xce')
        cmd = b'\xc1' + code + b'\0' + b'\0' + b'\0' + b'\0' + b'\0' + b'\0' + b'\xce' + cs
    ...
    def _get_response(self, timeout=None, wait_part=None):
        ...
        try:
            ...
            #            self._log_debug("CMD_A '%s', LEN %i, DATA %s, CRC %s" % (hex(byte2int(code)),
            #                                                                      length,
            #                                                                      ' '.join(hex(byte2int(c)) for c in data),
            #                                                                      hex(byte2int(chk_sum))
            #                                                                     ))
