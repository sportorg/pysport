...
+ Экспорт в формат SFRX с данными соревнования, дистанциями, участниками, результатами и сплитами
...
+ Добавлена поддержка мультиспортивных разрядных таблиц и настроек одновременно
+ Для ARDF выполнение разрядов согласно требований ЕВСК
+ На панели инструментов добавлена кнопка Онлайн (Live) с индикатором состояния и контекстным меню (Включить/Выключить, Отправить выбранные, Настройки)
+ В карточке участника добавлена кнопка для присвоения номера (максимальный номер в базе + 1)
+ В карточке результата добавлена кнопка для изменения номера участника на введённый
+ Добавлена вкладка подключенные клиенты в настройки Командой работы
+ Добавлено опциональное шифрование данных Teamwork (AES-256-GCM) с локальным хранением ключа и генерацией ключа в настройках
+ Настройки Teamwork (хост/порт/режим/шифрование) перенесены из файла соревнования в настройки программы; добавлен флаг "Запустить при старте" и автоматический fallback в режим клиента при старте, если сервер уже запущен
+ Добавлена проверка ID гонки в Teamwork на сервере, подтверждение на клиенте при несовпадении и подтверждение отключения Teamwork при переключении дня многодневки
+ Добавлена поддержка системы хронометража Huichang ([#550](https://github.com/sportorg/pysport/pull/550))
+ Переделан принцип начисления штрафа для заданного направления. Если участник пропустил 1 КП, получит 1 штраф. Также добавлена опция включать/выключать штраф за чужие КП
+ Добавлена конвертация наименований разрядов в различных форматах
+ Добавлено поле «Короткое название» в свойствах соревнования — отображается в заголовке окна и списке многодневки
+ Добавлены флаги функций в настройках с переключателем поддержки SFR для видимости меню
+ Добавлен переключатель темы оформления (Светлая / Тёмная / Системная)
+ В релизы вместе с MSI входят EXE-инсталлятор Inno Setup и portable-архив, инсталляторы предлагают 64-битный каталог `Program Files` и создают `{app}\data` и `{app}\logs` с правами на запись
+ Добавлена инструкция по сборке пакета, совместимого с Windows 7
...
+ Исправлена ошибка печати сплита, если у участника не найдена группа или дистанция ([#526](https://github.com/sportorg/pysport/issues/526))
+ Исправлен быстрый ввод номера забега с помощью Alt+число ([#536](https://github.com/sportorg/pysport/issues/536))
+ Исправлено: некорректное удаление строки при обновлении таблицы записей во время подтверждения
+ Исправлена командная работа: дублирование результатов при повторной отправке; удалённый объект не появлялся при повторной отправке
+ Исправлена потеря сортировки групп и организаций при отправке выбранных в шаблон (HTML/DOCX/CSV)
+ Исправлена потеря многодневных результатов при отправке выбранных в шаблон (HTML/DOCX/CSV)
+ Исправлено: финиш всегда 00:00:00 при любых настройках при отсутствии отметки для ТаймКонтур
+ Исправлена ошибка чтения бесконтактных чипов ТаймКонтур в случае отсутствия пакета уровня батареи в ответе мастер-станции
+ Исправлено: данные, логи, конфигурации, шаблоны и звуки ищутся относительно программы, а не рабочей директории
+ Исправлено: первый запуск больше не подменяет штатные значения настроек значениями из `config.ini`. Импорт `config.ini` выполняется, только если файл существует
+ Исправлено: Команда `uv run poe run` также выполняет `generate-mo` и `generate-version`
+ Исправлено: `builder.py bdist_msi` падал с `FCI error 1`, если имя упаковываемого файла содержало не-ASCII символы — например, шаблон отчёта с кириллицей в названии
+ Ускорена работа подсчета результатов в 3 раза
